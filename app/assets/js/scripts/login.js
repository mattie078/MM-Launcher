    ...
    if (args[0] === 'error') {
        ...
        switch (args[1]){
            ...
            case 'AuthNotFinished': {
                setOverlayContent('ERROR', 'You have to finish the login process to use Mystical Machines. The window will close by itself when you have successfully logged in.', 'OK')
                setOverlayHandler(() => {
                    toggleOverlay(false)
                    toggleOverlay(false, false, 'msOverlay')
                })
