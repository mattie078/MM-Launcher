...
const loggerSettings = LoggerUtil(
  "%c[Settings]",
  ...
  document.getElementById("settingsAddServerCode").onclick = () => {
    for (let ele of document.getElementsByClassName(
      ...
      if (!ConfigManager.getServerCodes().includes(code) && code) {
        ...
        loggerSettings.log(
          "Server code already exists or is empty, not adding."
        );
      }
    }
  };
    ...
    v.onclick = () => {
      if (v.hasAttribute("remcode")) {
        ...
        if (ConfigManager.getServerCodes().includes(code)) {
          ConfigManager.getServerCodes().splice(
            ConfigManager.getServerCodes().indexOf(code),
            1
          );
          ConfigManager.save();
          loggerSettings.log(
            "Added removed code from configuration and saved it"
          );
          prepareLauncherTab();
        }
      }
...
function resolveServerCodesForUI() {
  ...
  for (let servCode of ConfigManager.getServerCodes()) {
    ...
    servCodes += `
                <div id="${servCode}" class="settingsServerCode" ${
      valid ? "valid" : ""
    }>
