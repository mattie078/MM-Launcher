...
function randomiseBackground() {
  let backgroundDir = fs.readdirSync(
    path.join(__dirname, "assets", "images", "backgrounds")
  );
...
function checkCurrentServer(errorOverlay = true) {
  ...
  if (selectedServId) {
    const selectedServ = DistroManager.getDistribution().getServer(
      selectedServId
    );
    if (selectedServ) {
      if (selectedServ.getServerCode() && selectedServ.getServerCode() !== "") {
        if (
          !ConfigManager.getServerCodes().includes(selectedServ.getServerCode())
        ) {
          if (errorOverlay) {
            setOverlayContent(
