...
class AssetGuard extends EventEmitter {
  ...
  static _validateLocal(filePath, algo, hash) {
    if (fs.existsSync(filePath)) {
      ...
      if (hash == null) {
        console.log(
          "There's no hash clientside, we assume it doesn't exist and all is good!"
        );
        return true;
      }
        ...
        for (let i = 0; i < zipEntries.length; i++) {
          if (zipEntries[i].entryName === "version.json") {
            ...
            if (!fs.existsSync(versionFile)) {
              ...
              fs.writeFileSync(
                path.join(versionPath, forgeVersion.id + ".json"),
    ...
    return new Promise(async (resolve, reject) => {
      ...
      for (let ob of modules) {
        ...
        if (
          ...
          if (
            Util.isForgeGradle3(server.getMinecraftVersion(), ob.getVersion())
          ) {
            // Read Manifest
            for (let sub of ob.getSubModules()) {
              if (sub.getType() === DistroManager.Types.VersionManifest) {
                resolve(
                  JSON.parse(
                    fs.readFileSync(sub.getArtifact().getPath(), "utf-8")
                  )
                );
                return;
              }
...
*/
        ...
        (err) => {
          if (err) {
            ...
            console.log(
              "All " + identifier + " have been processed successfully"
            );
          }
